...
class Demo():
    ...
    def forward(self):
        ...
        for val in dict:
            ...
            self.net_map.load_state_dict(torch.load(map_net_pth, map_location=self.device,weights_only=True))

            seg_net_pth = getattr(self.opt, 'seg_'+val)
            self.net_seg.load_state_dict(torch.load(seg_net_pth, map_location=self.device,weights_only=True))
